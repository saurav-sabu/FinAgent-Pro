...
class FinanceAgent:
    ...
    async def stream_analyze(self, query: str):
        """
        Stream a financial query response using the AI agent.
        ...
        try:
            ...
            logger.info(f"Starting streamed analysis for: {query[:50]}...")
            
            # Run the agent asynchronously with streaming enabled
            response_stream = self.agent.arun(prompt, stream=True)
            async for chunk in response_stream:
                if chunk.content:
                    yield chunk.content
                    
        except Exception as e:
            logger.error(f"Error during streamed analysis: {str(e)}")
