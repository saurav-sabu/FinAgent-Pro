import { useState, useEffect } from 'react';
import { marketAPI } from '../services/api';
import SectorHeatmap from './SectorHeatmap';
import { PieChart, Loader2, AlertCircle } from 'lucide-react';

const HeatmapPanel = () => {
    const [sectors, setSectors] = useState({});
    const [loading, setLoading] = useState(true);
    const [error, setError] = useState(null);

    useEffect(() => {
        const fetchSectors = async () => {
            try {
                setLoading(true);
                // The backend bundles sector performance in the standard dashboard API.
                // We'll borrow it from the AAPL request structure for now to keep it efficient.
                const data = await marketAPI.getDashboard("AAPL");
                setSectors(data.sectorPerformance || {});
                setError(null);
            } catch (err) {
                console.error(err);
                setError("Failed to load sector performance heatmap.");
            } finally {
                setLoading(false);
            }
        };

        fetchSectors();
    }, []);

    if (loading) {
        return (
            <div className="flex flex-col items-center justify-center p-12 h-64">
                <Loader2 className="w-8 h-8 text-fin-accent animate-spin mb-4" />
                <p className="text-fin-muted">Scanning US Sector performance...</p>
            </div>
        );
    }

    if (error || Object.keys(sectors).length === 0) {
        return (
            <div className="flex flex-col items-center justify-center p-12 h-64 text-fin-red">
                <AlertCircle className="w-8 h-8 mb-4" />
                <p>{error || "No sector data available."}</p>
            </div>
        );
    }

    return (
        <div className="flex flex-col h-full animate-fade-in w-full">
            <div className="mb-6 p-4 rounded-xl bg-fin-bg border border-fin-border/50 flex items-center justify-between">
                <div>
                    <h2 className="text-xl font-bold text-fin-text flex items-center gap-2">
                        <PieChart className="w-5 h-5 text-fin-purple mr-1" />
                        US Sector Breakdown
                    </h2>
                    <p className="text-sm text-fin-muted mt-1">Relative performance of major SPDR Sector ETFs over 5 Days</p>
                </div>
            </div>

            <div className="bg-fin-bg rounded-xl p-6 border border-fin-border/50 shadow-sm flex-1">
                <SectorHeatmap sectors={sectors} />
            </div>
        </div>
    );
};

export default HeatmapPanel;
